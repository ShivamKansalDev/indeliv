...
export const paymentsApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    deletePayment: builder.mutation<{ success: boolean; id: number | string }, {id: number | string}>({
      query: ({id}) => ({
...
export const { useGetPaymentsMutation, useEditPaymentMutation, useDeletePaymentMutation, useVerifyPaymentMutation} = paymentsApiSlice;
