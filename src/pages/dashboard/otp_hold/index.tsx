...
export default function OTPHOLD() {
    ...
    async function handleOtp() {
        if (otpValue) {
            ...
            try {
                const response = await axios.post(
                    `${HOST[process.env.NODE_ENV]}/api/payments/otp?code=${otpValue}`,
                    {},
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data',
                ...
                if (response.status === 200) {
                    ...
                    response.data == "FAIL" && alert(`Wrong OTP entered. Please try again.`);
                    ...
                    response.data == "SUCCESS" && navigate(`${redirectPath}${invoiceData?.id}`);
                } else {
                    alert(`Failed to update invoice status: ${response?.data || 'Unknown error'}`);
                ...
                if (error.response) {
                    ...
                    errorMessage = `Error: ${error.response.data || 'An unexpected error occurred.'}`;
                        ...
                        <div className="otp-component ">
                            ...
                            <ul className="list-unstyled m-0 p-0 pt-md-4 px-md-4">
                                ...
                                <li className="pb-4" style={{ fontSize: "14px", fontWeight: "400", paddingTop: "20px" }} >
                                    ...
                                    <input type="number" name="otpValue" value={otpValue} onChange={handleInputChange} onKeyDown={handleKeyDown} className="form-control otp-input" id="otpHold" placeholder="Enter OTP" />
                                    <span style={{ color: "#0080FC", fontWeight: "600" }} className="d-inline-block text-decoration-underline">Re-send OTP</span>
                                </li>
                            </ul>
                            <hr className="d-none d-md-block m-0" style={{ color: "#b9b9b9" }} />
