...
export default function BatchDetailsTable({
  ...
  batchParamsId: string | undefined;
  setbatchData: any;
  ...
  const [deleteInvoiceFromBatch, { data: deleteData, error: deleteError }]: any = useDeleteInvoiceFromBatchMutation();
  const handleDeleteInvoices = async (invoiceId: number) => {
    ...
    try {
      ...
      const {data} = await batchDetailApi(batchParamsId)
      setbatchData(data)

      if (batchs?.invoices?.length !== 1) {
        const { isError } = await refetchBatch();
      }

      if (batchs?.invoices?.length === 1) {
        isCollection ? navigate("/dashboard/batches/collections") : navigate("/dashboard/batches/deliveries");
      }
  ...
  const paymentMap = new Map(batch?.payments?.map(item => [item.invoice_id, item.created_at]));

  const invoiceDetails = invoices?.map((list: any) => {
    const latestReturnCreatedAt = list?.returns?.[list?.returns?.length - 1]?.created_at;
    const createdAt = paymentMap.get(list.id) || latestReturnCreatedAt || list.created_at;
  ...
  return (
    <>
      <table className="batch-table-component">
        ...
        <tbody>
          {invoiceDetails?.map((invoice: Invoice, index: number) => (
            <tr
              key={index}
            ...
            >
              ...
              <div className="link-wrapper desktop-only">
                ...
                <td>{invoice?.buyer?.name} {!invoice?.buyer?.phone ? <OverlayTrigger
                  trigger={['hover', 'focus']}
                  placement="bottom"
                      ...
                      invoice.status == "1"
                        ...
                        : invoice.status == "2"
                          ...
                          : invoice.status == "5"
                            ? "On Hold"
                            : invoice.status == "4"
                              ? window.location.pathname?.includes('collection') ? invoice?.amount_due === 0 ? "Completed" : "Attempted" : "Completed"
                              : invoice.status == "3"
                                ? window.location.pathname?.includes('collection') ? batch?.payments?.find((item) => item?.invoice_id === invoice?.id && item?.method === 'Credit') ? "Attempted" : "Delivered" : "Delivered"
                                : ""
                    }
                  />
                </td>
                    ...
                    <div className="d-flex flex-column align-items-stretch">
                      ...
                      <div className="d-flex flex-column " >
                        <p className="fw-light" style={{ fontSize: "14px", margin: 0 }}>
                          {invoice?.buyer?.name}
                          {!invoice?.buyer?.phone ? <OverlayTrigger
                            trigger={['hover', 'focus']}
                            placement="bottom"
                            overlay={<Popover id="popover-trigger-hover-focus" title="Popover bottom" className="invoice-tooltip">
                              Mobile number for<br /> {invoice.buyer.name}<br /> is missing on Tally
