...
export default function BatchTable({
  ...
  useEffect(() => {
    ...
    batchs.map(val => {
      if (val.status === "3") {
        sortedBatch.push(val)
      } else {
        sortedBatch.unshift(val)
      }
    })
    setSortedBatch(sortedBatch)
  }, [batchs])
                ...
                >
                  ...
                  >
                    <BatchProgress
                      ...
                      completedCount={batch.status == "3" ? batch.total_invoices ?? 0 : batch.delivered_invoices ?? 0}
                      totalCount={batch.total_invoices ?? 0}
                      type={lastSegment}
                    />
                  </div>
