...
export default function Completed() {
  const { searchTxt, fromDate, toDate } = useInvoicesListState();
  ...
  useEffect(() => {
    ...
    if (!invoicesState.lastPageReached && loadMoreTrigger)
      getInvoices({
        status: 4, page: page + 1, keyword: searchTxt, sort_by: stateDataSorting.sortBy,
        sort_order: stateDataSorting.isAsc ? "DESC" : 'ASC',
        from: fromDate,
        to: toDate
      });
