...
export default function PaymentCollection() {
  ...
  const [totalDue, setTotalDue] = useState(location?.pathname?.includes("/collection") ? collectionAmount : deliveryAmount);
  useEffect(() => {
  ...
  async function paymentCollectionSubmit() {
    try {
      ...
      let cash = values?.cash
        ? "&cash=" + Number(values?.cash?.split(",").join(""))
        : "";
      let online = values?.online
        ? "&online=" + Number(values?.online?.split(",").join(""))
        : "";
      let cheque = values?.cheque
        ? "&cheque=" + Number(values?.cheque?.split(",").join(""))
        : "";
      let credit = values?.credit
        ? "&credit=" + Number(values?.credit?.split(",").join(""))
        : "";
        ...
        isImageUpload ?
          ...
          <>
            <div className="d-flex justify-content-center align-items-center scroll-bottom-delivery">
              <div className="otp-component mb-5 mb-md-0 pb-5 pb-md-0">
                ...
                <ul className="list-unstyled overflow-hidden">
                  ...
                  <li className="p-0 p-md-4">
                    <div className="payment-body">
                      ...
                      {/* ---for mobile */}
                      <div
                        className="to-be-collected  d-block d-md-none payment-body-head "
                        style={{ color: "#1D2939", padding: "15px 12px" }}
                      >
                        <div
