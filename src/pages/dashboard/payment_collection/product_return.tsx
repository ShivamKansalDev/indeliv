...
const HandleModelReturn: React.FC<ProductReturn> = ({ setPrice, setReturnItem, setProductValue, productValue }) => {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>, Id: number, productId: number, qty: number, unit_price: number, item_name: string) => {
          ...
          isImageUpload ?
                ...
                <div className="main-div">
                  <div className="header p-md-3">
                    <div className="d-flex">
                      ...
                      <span>{productValue[imageName] ? productValue[imageName]?.item_name : "Upload Image"}</span>
                    </div>
            ...
            <>
              <Modal.Header className="modal_head_payment">
                <Modal.Title className="w-100">
                  <div className="back-link">
                    <span className="d-block mobile-only-return">
                      <div className="d-flex">
                        <LeftArrow onClick={handleClose} className="arrow_icon align-self-center" />{" "}
                        <span> Product Returns</span>
                        <span className="selectedItemProduct d-flex justify-content-center" style={{ fontSize: "16px", fontWeight: "500" }}>
                          {total}
                        </span>
                      </div>
                    </span>
                    <div className="d-none desktop-only-return w-100">
              ...
              <Modal.Body className="modal_body">
                {/* <div className=""> */}
                <div className="pb-4 pb-md-0 d-block mobile-only-return product_return_mobile_modal overflow-scroll">
                    ...
                    <div className="w-100 product_return mb-3" key={index}>
                      ...
                      <div className="d-flex justify-content-between">
                        <div className="d-flex product_mob_productValue" style={{ padding: "12px" }}>
                          <label htmlFor={`check-${product?.id}`}>
                            <input
                              type="checkbox"
                              checked={productValue?.[product?.id]?.checked ? productValue?.[product?.id]?.checked : false}
                              id={`check-${product?.id}`}
                              className={`ivdt-checked ${productValue?.[product?.id]?.checked && productValue?.[product?.id]?.checked && "active"
                                }`}
                              onChange={(e) => handleCheckboxChange(e, product?.id, product?.id, product?.quantity, product?.unit_price, product?.item_name)}
                            />
                          </label>
                        ...
                        <div
                          ...
                          style={{
                            ...
                            color: productValue?.[product?.id]?.checked
                              ? qtyError?.includes(product?.id) ? "red" : "#0080FC" : "text-gray-return"
                          }}
                        >
                          <> {editMode[product?.id] ? (
                            <label className="cust-lable">
                              <span id="tpl" className="tpl">{` ${productValue?.[product?.id]?.quantity || ""} `}</span>
                                <input
                              type="number"
                              style={{
                                border: "none",
                                outline: "none",
                                color: qtyError?.includes(product?.id) ? "red" : "#0080FC",
                                background: "#F9FAFB"
                              }}
                              ...
                              // }
                              value={productValue?.[product?.id]?.quantity}
                              onChange={(e) =>
                                handleInputChanges(
                                  product?.id,
                              ...
                              onBlur={(e) => handleBlurReturn(e, product?.id)}
                              // onKeyDown={(event) =>
                            ...
                            productValue?.[product?.id]?.quantity ?
                              Number(productValue?.[product?.id]?.quantity).toFixed(1) :
                              "0.0"}
                            {" "} unit
                          </>
                          <span className="m-2">
                            {productValue?.[product?.id]?.checked ? (
                              <Pen onClick={() => handlePenClick(product?.id)} />
                            ) : (
                              <PenGray />
                            )}
                          </span>
                      ...
                      />
                        ...
                        <div>
                          ...
                          >
                            {productValue?.[product?.id]?.checked ?
                              productValue?.[product?.id]?.image_show ?
                                <BlueCamera style={{ height: "31px", width: "31px" }} onClick={() => uploadImageClick(true, String(product?.id))} />
                                : <BlueCameraUnchecked style={{ height: "31px", width: "31px" }} onClick={() => uploadImageClick(true, String(product?.id))} />
                ...
                <div className="d-none desktop-only-return">
                  <table
                  ...
                  >
                    ...
                    <tbody>
                      {invoiceData?.items?.map((product, index) => (
                        <tr key={index} className={`${productValue?.[product?.id]?.checked}`}>
                            ...
                            {productValue?.[product?.id]?.checked ? (
                              <Container
                                ...
                                style={{
                                  width: "60px",
                                  height: "36px",
                                  border: qtyError?.includes(product?.id) ? "1px dashed  red" : "1px dashed  #0080FC",
                                  borderRadius: "8px",
                                  padding: "10px 16px",
                                  margin: "0px",
                                  display: "inline-block"
                                }}
                                onClick={() => handlePenClick(product?.id)}
                              //   onClick={handleClick}
                              >
                                {/* {editMode[product?.id] ? ( */}
                                <input
              ...
              <div className="payment-footer-mob-modal mobile-only-return w-100">
