...
const getVariant = (status: string) => {
    switch (status) {
        ...
        case 'Completed':
            return 'success';
        case window.location.pathname.includes('delivery') ? 'Delivered' : 'Attempted':
            return 'success';
        default:
            return 'secondary';
    }
...
export function BatchStatusDelivery({ batchStatus }: { batchStatus: string }) {
    ...
    // @ts-ignore
    const currentStyle = styles[window.location.pathname.includes('collection') ? (batchStatus === 'Attempted' || batchStatus === 'Delivered') ? 'success' : variant : variant] || {};
    return (
...
export default function DeliveryDetailsTable({
    ...
    console.log('batchsState', batchs)
    ...
    const redirectOtp = (item: any) => {
        // console.log(item,"------")
        const isAttempted =  batch?.payments?.find((list) => list.invoice_id == item?.id && list?.method === 'Credit')
        if (isAttempted?.id || (!isCollection && item?.status == "3") || item?.status == "4") {
            console.log("Delivered")
        } else {
            navigate(`${isCollection ? "/dashboard/collection/otp_hold/" : "/dashboard/delivery/otp_hold/"}` + item?.id)
        }
    }
    function areAllInvoicesPresent(invoices: any, payments: any) {
        // Create a set of invoice ids from the invoices array
        const invoiceIds: any = new Set(invoices.map((invoice: any) => invoice.id));
        
        // Create a set of invoice ids from the payments array
        const paymentInvoiceIds: any = new Set(payments.map((payment: any) => payment.invoice_id));
        
        // Check if all invoice ids are present in the paymentInvoiceIds set
        return [...invoiceIds].every(id => paymentInvoiceIds.has(id));
    }
    
    const result = areAllInvoicesPresent(batchs?.invoices, batchs?.payments);
    return (
        <>
                        ...
                        >
                                                ...
                                                : invoiceItem.status == "2"
                                                    ...
                                                    : invoiceItem.status == "5"
                                                        ...
                                                        : invoiceItem.status == "3"
                                                            ? window.location.pathname.includes('delivery') ? "Delivered" : batch?.payments?.find((item) => item?.invoice_id === invoiceItem?.id && item?.method === 'Credit') ? "Attempted" : "Ongoing"
                                                            : invoiceItem.status == "4"
                                                                ? window.location.pathname.includes('delivery') ? "Completed" : invoiceItem?.amount_due === 0 ? "Completed" : "Attempted"
                                                                : ""
                                        }
                                    />
                                </td>
                            ...
                            </div>
                            {
                                batch?.associate?.role_name?.includes(permissions?.role_name) ?
                                    ...
                                    >
                                        <div className="title-area-delivery bg-white" style={{ padding: "12px" }}>
