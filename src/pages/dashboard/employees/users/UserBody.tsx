...
const UserBody: React.FC = () => {
  ...
  const [deleteSelectedUser, setDeleteSelectedUser] = useState<User|null>(null);
  const [activeDeactiveUser, setActiveDeactiveUser] = useState<User|null>(null);
...
useEffect(() => {
  ...
  async function getUsersList(changedUser: User | null = null){
    try {
      ...
      if(searchText && changedUser){
        const extractUpdatedUser = data.find((item) => item.id === changedUser.id);
  ...
  async function createUpdateUserAPI(){
    try {
      ...
      let data: string = (editSelectedUser)? `update/${editSelectedUser?.id}}` : "create";
      if(editSelectedUser){
      ...
      console.log("@@@ UPDATE USER: ", croppedImage, JSON.stringify(formData, null, 4));
      // return;
      ...
      if(error?.response?.status === 401){
        alert(error?.response?.data?.message || JSON.stringify(error?.response?.data, null, 4))
      }
    }
  }
      ...
      <div key={`${item?.id}userbody`} className="employee-card">
        <div className="details">
          <div className="d-flex gap-3 gapImg">
            <img src={item?.image_path || "/assets/image/no_image.jpeg"} alt="user_image"  className="employee_image" />
            <div className="name">
        ...
        <div className="options">

            {
              ...
              <>
               {(item?.is_active) ?
                  <button className="activate" onClick={() => { setActiveDeactiveUser(item) }}>
                  {(item?.is_active) ? "Deactivate" : "Activate"}
                </button>
              : 
                <button className="deactivate" onClick={() => { setActiveDeactiveUser(item) }}>
                    {(item?.is_active) ? "Deactivate" : "Activate"}
                </button>
                }
              </>
                ...
                {(canEdit)? (
                  ...
                  (
                    <div style={{width: '100px'}}/>
                     ...
                     {(canDelete)? (
                          <button className="delete" onClick={()=>{setDeleteSelectedUser(item)}}>
                            Delete
                          </button>
                        )
                        :
    ...
    <>
      ...
      <div className="employee-detail-page bg-pure-white">
        ...
        <ActiveDeactiveModal 
          ...
          updateUserStatus={() => {
            const data = `id=${activeDeactiveUser?.id}&status=${!activeDeactiveUser?.is_active}`;
        ...
        {(informationOpen || (editSelectedUser && userDetails)) && (
          <InformationModal
            ...
            setUserDetails={(details: UserDetails) => setUserDetails({
              ...userDetails,
              ...details
            })}
